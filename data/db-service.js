...
export const createCategoriesTable = async db => {
  const query =
    'CREATE TABLE IF NOT EXISTS categories (name TEXT, categoryType TEXT)';
  await db.executeSql(query);
};
...
export const createTransactionsTable = async db => {
  const query =
    'CREATE TABLE IF NOT EXISTS transactions (transactionDate INTEGER, summary TEXT, category_id INTEGER NOT NULL, transactionType TEXT, amount FLOAT)';
  await db.executeSql(query);
};
